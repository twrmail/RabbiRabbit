...
export default function StudyForm({ onStudy, loading, setLoading, error, setError }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateStudy({
        studyType, primaryInput, additionalVerses,
        character, theme, audience, translation, sections, notes,
      })
      onStudy(result)
    } catch (e) {
    ...
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <Input
          ...
          placeholder={PLACEHOLDERS[studyType]}
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div>
          ...
          <Input
            value={additionalVerses}
            onChange={e => setAdditionalVerses(e.target.value)}
            placeholder="e.g. Isaiah 53:5, Hebrews 4:15"
            multiline rows={2}
          />
        </div>
        <div>
          ...
          <Input
            ...
            placeholder="e.g. Paul, Miriam, Peter"
          />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <Input
          ...
          placeholder="e.g. redemption, courage in suffering, the kingdom of God"
        />
      </div>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {AUDIENCE.map(a => (
            <Chip key={a.value} label={a.label} selected={audience === a.value} onClick={() => setAudience(a.value)} />
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {TRANSLATIONS.map(t => (
            <Chip key={t.value} label={t.label} selected={translation === t.value} onClick={() => setTranslation(t.value)} />
          ))}
        </div>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {SECTIONS.map(s => (
            <Chip key={s.value} label={s.label} selected={sections.includes(s.value)} onClick={() => toggleSection(s.value)} />
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 26 }}>
        ...
        <Input
          ...
          placeholder="Specific angles, questions your group is wrestling with, things to include or avoid…"
          multiline rows={3}
        />
      </div>
      ...
      >
        {loading ? 'Preparing your study… this may take a moment' : 'Generate Bible Study →'}
      </button>
